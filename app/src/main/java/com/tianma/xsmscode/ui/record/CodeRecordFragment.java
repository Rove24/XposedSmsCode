...
public class CodeRecordFragment extends DaggerBackPressFragment implements CodeRecordContract.View {
    ...
    private void showSmsDetails(final RecordItem recordItem) {
        ...
        View dialogView = LayoutInflater.from(mActivity).inflate(R.layout.dialog_m3_sms_details, null);
        ...
        android.widget.TextView tvSenderDate = dialogView.findViewById(R.id.tv_details_sender_date);
        String sender = !android.text.TextUtils.isEmpty(smsMsg.getCompany()) ? smsMsg.getCompany() : smsMsg.getSender();
        String dateStr = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss", java.util.Locale.getDefault()).format(new java.util.Date(smsMsg.getDate()));
        tvSenderDate.setText(sender + " · " + dateStr);

        android.widget.TextView tvBody = dialogView.findViewById(R.id.tv_details_body);
        tvBody.setText(smsMsg.getBody());

        View btnCancel = dialogView.findViewById(R.id.btn_details_cancel);
        btnCancel.setOnClickListener(v -> dialog.dismiss());

        View btnCopySms = dialogView.findViewById(R.id.btn_details_copy_sms);
        btnCopySms.setOnClickListener(v -> {
            copySms(recordItem);
            dialog.dismiss();
        });

        View btnCopyCode = dialogView.findViewById(R.id.btn_details_copy_code);
