...
public class SmsCodeUtils {
    ...
    private static String loadCodeKeywordsByXSP() {
        try {
            ...
            return PrefConst.SMSCODE_KEYWORDS_DEFAULT;
        }
    }
     ...
     */
    public static String parseSmsCodeIfExists(Context context, String content, SharedPreferences sp) {
        String result = parseByCustomRules(context, content);
        if (TextUtils.isEmpty(result)) {
            result = parseByDefaultRule(context, content, sp);
        }
        return result;
    }
    ...
    private static String parseByDefaultRule(Context context, String content, SharedPreferences sp) {
        String keywordsRegex = "";
        if (sp != null) {
            keywordsRegex = XSPUtils.getSMSCodeKeywords(sp);
        }
        if (TextUtils.isEmpty(keywordsRegex) && context != null) {
            keywordsRegex = loadCodeKeywordsBySP(context);
        }
        if (TextUtils.isEmpty(keywordsRegex)) {
            keywordsRegex = PrefConst.SMSCODE_KEYWORDS_DEFAULT;
        }
    ...
    private static String parseByDefaultRule(Context context, String content, boolean useXSP) {
        if (useXSP) {
            return parseByDefaultRule(context, content, PrefBridge.loadPreferences(context));
        } else {
            return parseByDefaultRule(context, content, (SharedPreferences) null);
        }
    }
