    ...
    private void sendHeartbeat(Context context, String fromPkg) {
        ...
        new Thread(() -> {
            try {
                ...
                XLog.i("Sent legacy activation heartbeat from " + fromPkg);
            } catch (Throwable t) {
                XLog.w("Failed to send legacy activation heartbeat: " + t.getMessage());
            }
        }, "xsmscode-legacy-heartbeat").start();
    }
