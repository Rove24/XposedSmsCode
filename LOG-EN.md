# Update Logs
- 26.09.05 v0.0.13 (Version 13)
  1. Refactor: Modern Material Design 3 UI redesign (pixel-perfect alignment with Google Messages settings UX, full Android 12+ Monet dynamic theming support).
  2. Refactor: Upgraded to LibXposed Modern API 102 architecture with LSPosed static scope (Phone Services + System Framework).
  3. Feature: Zero-delay cross-process real-time preference sync (Intent Bundle channel) - changes take effect in 0ms without reboots.
  4. Fix: Resolved false "not activated" status after reinstall/reboot (system-level Settings.Global state + multi-layer lifecycle Context capture).
  5. Fix: Android 14+ receiver export restrictions (RECEIVER_EXPORTED) and notification channel compliance.
