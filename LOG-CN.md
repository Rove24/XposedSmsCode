...
- 26.09.05 v0.0.13 (Version 13)
  1. 重构: 全面升级至现代 Material Design 3 风格（完全像素级对齐 Google 信息设置体验，支持 Android 12+ Monet 动态取色）。
  2. 重构: 升级至现代 LibXposed API 102 架构，默认启用 LSPosed 静态作用域（电话服务 + 系统框架）。
  3. 功能: 新增全配置项跨进程实时热同步（Intent Bundle 通道），修改开关 0ms 立即生效，告别频繁重启。
  4. 修复: 彻底解决卸载重装开机后误报“未激活”问题（系统级 Settings.Global 状态共享 + 多层生命周期 Context 捕获）。
  5. 修复: 适配 Android 14+ 广播导出控制（RECEIVER_EXPORTED）与通知渠道规范。
